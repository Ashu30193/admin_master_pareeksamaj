...
const HigherEducation = ({ dispatch, siteConfig, getLoading }) => {
  ...
  return (
      ...
      onFinish={(event) => {
        Promise.all([
          dispatch({
            ...
            payload: {
              pathParams: { key: 'higher_qualification' },
              body: {
                higher_qualification: [
                  ...siteConfig.siteConfig.higher_qualification,
                  {
                    name:
    ...
    >
      <CardSection
        ...
        leftContent={
          <div className="pr-8 ">
            <div className="text-blue-900 font-semibold text-xl">Higher Qualification</div>
            <div className="text-gray-600">
              <p className="mt-4">Enter Higher Qualification Tags</p>
            </div>
          </div>
        }
        rightContent={
          <div className="p-4 bg-white shadow rounded">
            <div className="tags-input">
              <div className="flex flex-wrap mb-4">
                {siteConfig?.siteConfig?.higher_qualification.map((tag, index) => (
                  // eslint-disable-next-line react/no-array-index-key
                  <div key={index} className="font-semibold bg-gray-200 rounded-full px-4 py-1 m-1">
                    {tag.name}
                    <CloseOutlined
                      onClick={() => {
                        const newArray = siteConfig.siteConfig.higher_qualification;
